...
function citysyncai_create_city_page($city, $state) {
    ...
    ob_start();
    citysyncai_generate_ai_content($provider, $content_type, $city_full);
    $ai_content = ob_get_clean();

    // Extract just the content (remove wrapper divs)
    $ai_content = preg_replace('/<div[^>]*class=[\'"]citysyncai-ai-content[\'"][^>]*>.*?<\/div>/s', '', $ai_content);
    $ai_content = preg_replace('/<h3>.*?<\/h3>/s', '', $ai_content);
    $ai_content = preg_replace('/<p><em>.*?<\/em><\/p>/s', '', $ai_content);
