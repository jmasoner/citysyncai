...
add_action('wp_dashboard_setup', function () {
    wp_add_dashboard_widget('citysyncai_dashboard', 'CitySyncAI Status', 'citysyncai_render_dashboard_widget');
});

function citysyncai_render_dashboard_widget() {
    $provider = get_option('citysyncai_ai_provider', 'openai');
    $schema_type = get_option('citysyncai_schema_type', 'LocalBusiness');
    $ai_enabled = get_option('citysyncai_enable_ai', 'no');

    echo "<p><strong>AI Provider:</strong> $provider</p>";
    echo "<p><strong>Schema Type:</strong> $schema_type</p>";
    echo "<p><strong>AI Enabled:</strong> " . ($ai_enabled === 'yes' ? '✅' : '❌') . "</p>";

    echo '<p><a href="' . esc_url(rest_url('citysyncai/v1/flush-cache')) . '" class="button">Flush Cache</a></p>';
}