...
function citysyncai_register_settings() {
    ...
    add_settings_field('ai_provider', 'AI Provider', function () {
        $provider = get_option('citysyncai_ai_provider', 'deepseek');
        echo '<select name="citysyncai_ai_provider">';
        $providers = [
            'deepseek' => 'DeepSeek (RECOMMENDED - Very Affordable: $0.14/1M tokens)',
            'grok' => 'Grok (Alternative option)',
            'gemini' => 'Gemini (If you have tokens available)',
            'openai' => 'OpenAI',
            'claude' => 'Claude',
            'genspark' => 'Genspark',
        ];
        ...
        echo '</select>';
        echo '<p class="description"><strong>DeepSeek recommended</strong> for bulk generation - extremely affordable at ~$0.001-0.002 per city page. Perfect when you\'re out of Gemini tokens!</p>';
    }, 'citysyncai', 'citysyncai_main');
    ...
    add_settings_field('gemini_model', 'Gemini Model', function () {
        ...
        $models = [
            'gemini-pro' => 'Gemini Pro (Most Compatible - Recommended to start)',
            'gemini-1.5-flash' => 'Gemini 1.5 Flash (Fastest, Most Cost-Effective - Try if Pro works)',
            'gemini-1.5-pro' => 'Gemini 1.5 Pro (Highest Quality - Try if Pro works)',
        ];
        foreach ($models as $key => $label) {
            echo '<option value="' . esc_attr($key) . '"' . selected($model, $key, false) . '>' . esc_html($label) . '</option>';
        }
        echo '</select>';
        echo '<p class="description"><strong>Start with Gemini Pro</strong> for compatibility. If you get errors, try a different model. Flash is cheaper for bulk generation.</p>';
    }, 'citysyncai', 'citysyncai_main');
    ...
    add_settings_field('deepseek_key', 'DeepSeek API Key (RECOMMENDED for Bulk)', function () {
        ...
        echo '<p class="description"><strong>RECOMMENDED!</strong> Extremely affordable: $0.14 per 1M input tokens, $0.28 per 1M output tokens. ~$0.001-0.002 per city page. Get your key at <a href="https://platform.deepseek.com/" target="_blank">DeepSeek Platform</a></p>';
    }, 'citysyncai', 'citysyncai_api_keys');

    add_settings_field('deepseek_key_2', 'DeepSeek API Key (Secondary - Optional)', function () {
        $key = get_option('citysyncai_deepseek_key_2', '');
        echo '<input type="password" name="citysyncai_deepseek_key_2" value="' . esc_attr($key) . '" class="regular-text" />';
        echo '<p class="description">Add a second DeepSeek API key to double your capacity. Plugin will rotate between keys automatically.</p>';
    }, 'citysyncai', 'citysyncai_api_keys');
    ...
    add_settings_section('citysyncai_preview_section', 'AI Preview', function () {
        ...
        if ($enabled === 'yes') {
            if (empty($api_key)) {
                ...
                // Clear cached preview if it exists (so we always get fresh content)
                $cache_key = 'citysyncai_' . md5($provider . '_' . $type . '_Sample City, TX');
